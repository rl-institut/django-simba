...
class OpportunityChargingTimeseriesTestCase(TransactionTestCase):
    ...
    def build_opportunity_charging_scenario(self):
        ...
        The earliest departure is nudged off the whole minute first. SimBA anchors its simulation
        step grid at `get_departure_of_first_trip() - signal_time_dif`, so a schedule whose trips
        all sit on whole minutes -- as this fixture's do -- produces a grid that happens to be
        minute-aligned, event times land exactly on it, and nothing drifts. Real schedules carry
        seconds (the scenario this was found on departs at :15), which puts the grid out of phase
        with the minute-aligned events and is the condition the drift needs. Moving one departure
        45s earlier keeps it the earliest trip and reproduces it: without the fix, every resulting
        charging event is sampled +15s outside its own bounds.
        """
